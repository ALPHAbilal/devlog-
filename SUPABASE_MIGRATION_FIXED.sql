-- ============================================
-- CRITICAL SECURITY FIXES FOR DEVLOG
-- Date: 2025-01-29
...
-- PART 3: FIX FUNCTION SEARCH PATHS (SECURITY CRITICAL)
-- ============================================
...
-- 2. save_document_blocks_v3 (with complete implementation)
DROP FUNCTION IF EXISTS public.save_document_blocks_v3(uuid, uuid, text, text[], jsonb);
...
DROP FUNCTION IF EXISTS public.user_document_ids();
...
DROP FUNCTION IF EXISTS public.update_document_cache(uuid);
...
-- PART 4: FIX RLS POLICIES FOR PERFORMANCE
-- ============================================
...
BEGIN
    ...
    RAISE NOTICE 'Backup tables moved to backups schema: %', 
        (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'backups');
    RAISE NOTICE 'Functions with search_path fixed';
