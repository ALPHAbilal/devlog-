-- Fix batch_sync_changes to properly handle multiple updates to same block
-- Problem: Updates processed in array order, not timestamp order
-- Solution: Sort changes by timestamp before processing
...
BEGIN
  -- CRITICAL FIX: Sort changes by timestamp BEFORE processing
  -- This ensures newer updates always override older ones, even within same batch
  SELECT jsonb_agg(elem ORDER BY (elem->>'timestamp')::BIGINT ASC)
  INTO v_sorted_changes
  FROM jsonb_array_elements(p_changes) AS elem;

  -- Process all changes in timestamp order within single transaction
    ...
    BEGIN
      ...
      CASE v_change->>'action'
        WHEN 'UPDATE', 'CREATE' THEN
          ...
          WHERE blocks.updated_at <= EXCLUDED.updated_at;  -- Changed < to <= to handle same-timestamp updates
...
COMMENT ON FUNCTION public.batch_sync_changes IS
'Processes batch of block changes with timestamp-based ordering.
CRITICAL FIX (2025-11-09): Sorts changes by timestamp before processing to ensure
newer updates always override older ones, even when multiple updates to same block
are in the same batch. This fixes AI block message loss and FileTree data loss issues.';
