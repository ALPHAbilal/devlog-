-- Migration: Performance Optimizations for Devlog (Corrected)
-- Created: 2025-01-31
-- Phase: 2 - Performance
-- 
...
-- ============================================
-- PART 0: CLEANUP EXISTING CONFLICTING OBJECTS
-- ============================================

-- Drop existing versions of get_documents_with_stats
DROP FUNCTION IF EXISTS get_documents_with_stats(p_user_id uuid, p_limit integer);
DROP FUNCTION IF EXISTS get_documents_with_stats(page_size integer, page_offset integer, sort_by text, sort_desc boolean, filter_template boolean, filter_tags text[]);

-- Drop existing trigger functions (they will be recreated)
DROP FUNCTION IF EXISTS update_document_search_vector() CASCADE;
DROP FUNCTION IF EXISTS update_block_search_vector() CASCADE;
...
-- Note: search_vector columns already exist, so we'll just update them
...
-- Update existing documents and blocks that have NULL search vectors
